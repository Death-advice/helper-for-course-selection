...
r = q.WordArray = v.extend({
    ...
    concat: function(a) {
        ...
        a = a.sigBytes;
        this.clamp();
        if (g % 4) for (var p = 0; p < a; p++) e[g + p >>> 2] |= (d[p >>> 2] >>> 24 - p % 4 * 8 & 255) << 24 - (g + p) % 4 * 8;
        else if (65535 < d.length) for (p = 0; p < a; p += 4) e[g + p >>> 2] = d[p >>> 2];
    ...
    random: function(a) {
        for (var e = [], d = 0; d < a; d += 4) e.push(4294967296 * n.random() | 0);
...
w = x.Hex = {
    ...
    parse: function(a) {
        for (var e = a.length,
        d = [], g = 0; g < e; g += 2) d[g >>> 3] |= parseInt(a.substr(g, 2), 16) << 24 - g % 8 * 4;
...
b = x.Latin1 = {
    ...
    parse: function(a) {
        for (var e = a.length,
        d = [], g = 0; g < e; g++) d[g >>> 2] |= (a.charCodeAt(g) & 255) << 24 - g % 4 * 8;
...
n.enc.Base64 = {
    ...
    parse: function(c) {
        ...
        for (var n = [], r = 0, x = 0; x < q; x++) if (x % 4) {
...
w = r.Hasher,
r = v.algo,
b = [], y = 0; 64 > y; y++) b[y] = 4294967296 * n.abs(n.sin(y + 1)) | 0;
r = r.MD5 = w.extend({
    _doReset: function() {
...
u = f.EvpKDF = c.extend({
    ...
    compute: function(c, n) {
        for (var f = this.cfg,
        r = f.hasher.create(), b = q.create(), u = b.words, t = f.keySize, f = f.iterations; u.length < t;) {
            v && r.update(v);
            var v = r.update(c).finalize(n);
            r.reset();
            for (var a = 1; a < f; a++) v = r.finalize(v),
            r.reset();
            b.concat(v)
        }
        b.sigBytes = 4 * t;
...
c.BlockCipher = w.extend({
    ...
    reset: function() {
        ...
        a = d.iv,
        d = d.mode;
        if (this._xformMode == this._ENC_XFORM_MODE) var b = d.createEncryptor;
        else b = d.createDecryptor,
...
f = n.lib.BlockCipher,
c = n.algo,
q = [], u = [], v = [], r = [], x = [], w = [], b = [], y = [], t = [], A = [], a = [], e = 0; 256 > e; e++) a[e] = 128 > e ? e << 1 : e << 1 ^ 283;
for (var d = 0,
g = 0,
e = 0; 256 > e; e++) {
...
c = c.AES = f.extend({
    _doReset: function() {
        ...
        d = a.words,
        c = a.sigBytes / 4,
        a = 4 * ((this._nRounds = c + 6) + 1), e = this._keySchedule = [], f = 0; f < a; f++) if (f < c) e[f] = d[f];
        else {
            var g = e[f - 1];
    ...
    _doCryptBlock: function(a, b, c, d, e, f, g, h) {
        ...
        k = a[b] ^ c[0], l = a[b + 1] ^ c[1], n = a[b + 2] ^ c[2], p = a[b + 3] ^ c[3], q = 4, r = 1; r < m; r++) var t = d[k >>> 24] ^ e[l >>> 16 & 255] ^ f[n >>> 8 & 255] ^ g[p & 255] ^ c[q++],
...
///////////////////////////////////////////////
function getpwd(password, saltvalue) {

return encryptPassword(password, saltvalue);
